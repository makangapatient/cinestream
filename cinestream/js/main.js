...
(function () {
  ...
  function buildYearButtons() {
    ...
    panel.querySelectorAll('.year-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const filtered = year === 'all' ? ALL_CONTENT : ALL_CONTENT.filter(m => m.year === parseInt(year));
  ...
  $$('.filter-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      if (tabName === 'popular') {
        renderGrid('latestGrid', [...ALL_CONTENT].sort((a, b) => b.rating - a.rating), 20);
  ...
  $$('.genre-btn[data-letter]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const filtered = letter === 'all' ? ALL_CONTENT : ALL_CONTENT.filter(m => m.title.toUpperCase().startsWith(letter));
      renderGrid('latestGrid', filtered, 20);
      updateSectionTitle(letter === 'all' ? '🔤 All Titles' : `🔤 "${letter}"`);
      scrollToLatest();
    });
  });
  ...
  function openModal(item) {
    ...
    const related = ALL_CONTENT.filter(m => m.genre === item.genre && m.id !== item.id)
    ...
    wBtn.onclick                   = () => {
      ...
      if (idx === -1) { list.push(item.id); wBtn.textContent = '✓ In Watchlist'; wBtn.classList.add('added'); showToast('✅ Added to Watchlist'); }
      else            { list.splice(idx,1); wBtn.textContent = '+ Watchlist';    wBtn.classList.remove('added'); showToast('Removed from Watchlist'); }
  ...
  function loadServer(index, screen) {
    ...
    if (!url || url.includes('undefined')) {
      screen.innerHTML = `<div class="player-placeholder"><div class="player-logo">▶</div><p style="color:#fff">Not available on this server</p><p style="color:#7a7a90;font-size:13px">Try another server below</p></div>`;
      return;
    }
  ...
  function openPlayer(item) {
    console.log('[Player] Opening:', item ? item.title : 'undefined', '| ID:', item ? item.id : 'undefined');
    ...
    if (overlay) {
      const resetUiTimer = () => {
        ...
        uiHideTimer = setTimeout(() => overlay.classList.add('hide-ui'), 3500);
      };
  ...
  document.addEventListener('click', e => {
    ...
    if (page === 'trending') { items = ALL_CONTENT.filter(m => m.tags && m.tags.includes('Trending')); title = '🔥 Trending'; }
  ...
  /* ====================================================
     BOOT — wait for data then render
  ==================================================== */
  ...
  // Fallback if data already loaded
